# 🧵 LUMI – Nón lá cách tân

## 👥 Thông tin nhóm

| STT | Họ tên | MSSV | Vai trò |
|-----|--------|------|--------|
| 1 | Võ Thị Ly Ly | 24126125 | Code & Deploy |
| 2 | Lê Trần Trà My | 24126134 | Design & UI |
| 3 | Nguyễn Mai Khánh Linh | 24126115 | Design & Content |
| 4 | Phạm Nguyễn Thanh Mai | 24126127 | Test & Report |
| 5 | Nguyễn Thị Trúc Ly | 24126122 | Test & Quality |
| 6 | Lê Việt Nhật | 24126159 | Code & Logic |

---

## 📌 Mô tả dự án

- **Tên thương hiệu:** LUMI – Nón lá cách tân  
- **Slogan:** "Đội chất riêng – Chạm nét Việt"  
- **Chủ đề:** Website thương mại điện tử (E-commerce) bán nón lá truyền thống cải tiến  

### 🔥 Tính năng nổi bật
- Giao diện hiện đại, responsive (Desktop, Tablet, Mobile)  
- Danh mục sản phẩm + filter + phân trang  
- Giỏ hàng realtime  
- Form validation (đăng ký / thanh toán)  
- Tối ưu hiệu suất, SEO, deploy bằng GitHub Actions  

---

## 🔗 Link dự án

- 🎨 Figma (view-only): [\[Add link\]](https://www.figma.com/design/h5cigrWrKWJOy10WYGAcGt/Untitled?node-id=0-1&t=6jC02iX5ju0fvOQa-1)
- 💻 GitHub: [\[Add link\]](https://github.com/vietnhat1507/conical-hat-e-commerce)
- 🌐 Home Pages: [\[Add link\]](https://conical-hat-e-commerce.vercel.app/)
- 🎥 YouTube (nếu có): [[[Add link]]([https://www.youtube.com/watch?v=FlYBIZJ-Lz0)](https://www.youtube.com/watch?v=oetHtP3CBoY](https://www.youtube.com/watch?v=oetHtP3CBoY))

---

## ⚙️ Hướng dẫn chạy local

```bash
npm install
npm run dev
